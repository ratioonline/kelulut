...
export default function MediaManager({
  ...
  const [targetFolderUpload, setTargetFolderUpload] = useState(defaultFolder)
  ...
  useEffect(() => {
    if (isOpen) {
      scanExistingAppImages()
      setTargetFolderUpload(defaultFolder)
    }
  ...
  }, [isOpen, targetFolderUpload, moduleName])
  ...
  return (
    <>
      <Modal isOpen={isOpen} onClose={onClose} title="Media Manager" size="xl">
          ...
          {activeTab === 'library' && (
            <div className="space-y-4">
              ...
              {filteredItems.length === 0 ? (
                ...
                /* LIST VIEW */
                <div className="divide-y divide-gray-100 max-h-[420px] overflow-y-auto border border-gray-200 rounded-2xl bg-white">
                  {filteredItems.map((item) => {
                    const isSelected = selectedIds.includes(item.id)
                    return (
                      <div
