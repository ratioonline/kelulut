...
export default function CartDrawer() {
  ...
  const waMsg = (vendorId: string | null) => {
    const vendorItems = items.filter(i => ((i.product as any).umkm_id || null) === vendorId)
    if (vendorItems.length === 0) return

    const lines = vendorItems.map(
      (i) =>
        `• ${i.product.name} x${i.quantity} = ${formatCurrency(
          (i.product.discount_price ?? i.product.price ?? 0) * i.quantity
        )}`
    )
    const subtotal = vendorItems.reduce((acc, i) => acc + (i.product.discount_price ?? i.product.price ?? 0) * i.quantity, 0)
    const text =
      `Halo, saya ingin memesan:\n${lines.join('\n')}\n\nTotal: ${formatCurrency(subtotal)}\n\nMohon konfirmasi ketersediaan stok. Terima kasih!`

    const vendorWa = vendorId && vendorItems[0] && (vendorItems[0].product as any).umkm?.whatsapp 
      ? (vendorItems[0].product as any).umkm.whatsapp 
      : '6282272611515'
    
    const cleanWa = vendorWa ? vendorWa.replace(/\\D/g, '') : '6282272611515'

    window.open(`https://wa.me/${cleanWa}?text=${encodeURIComponent(text)}`, '_blank')
  }

  const uniqueVendors = Array.from(new Set(items.map(i => ((i.product as any).umkm_id || null))))
          ...
          <div className="border-t border-gray-100 px-5 py-4 space-y-3">
            ...
            <div className="space-y-2">
              {uniqueVendors.map(vid => {
                const vendorName = vid ? (items.find(i => (i.product as any).umkm_id === vid)?.product as any)?.umkm?.name || 'UMKM' : 'Official Kebun Kelulut'
                return (
                  <button
                    key={vid || 'official'}
                    onClick={() => waMsg(vid)}
                    className="w-full bg-green-500 hover:bg-green-600 active:scale-95 text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2 transition-all text-sm"
                  >
